...
class TestRetellUnits(SecondsVsMillisecondsTests):
    ...
    def test_latency_values_are_milliseconds(self) -> None:
        ...
        e2e = next(
            e
            for e in events
            if isinstance(e, StageObserved) and e.stage is Stage.E2E and e.turn_index == 0
        )
...
def test_vapi_reads_published_latency_keys() -> None:
    ...
    assert any(
        e.stage is Stage.LLM and e.metric is Metric.TTFT and e.value_ms == 320 for e in stages
    )
...
def test_retell_signature_uses_body_plus_timestamp() -> None:
    ...
    ok = plugin.authenticate(
        raw, RawHeaders.from_mapping({"x-retell-signature": f"v={ts},d={sig}"}).as_list(), cfg
    )
    assert ok.ok
    missing = plugin.authenticate(
        raw,
        RawHeaders.from_mapping({"x-retell-signature": f"v={ts},d={sig}"}).as_list(),
        cfg.model_copy(update={"secrets": {}}),
    )
...
def test_vapi_legacy_secret_fail_closed() -> None:
    ...
    cfg = ConnectionConfig(
        org_id="acme", provider="vapi", connection_id="c", ingest_key_hash="x", secrets={}
    )
    result = plugin.authenticate(
        raw, RawHeaders.from_mapping({"x-vapi-secret": "nope"}).as_list(), cfg
    )
...
def test_elevenlabs_coarse_message_anchors() -> None:
    ...
    plugin = ElevenLabsPlugin()
    events = list(
        plugin.decode(_env(ELEVEN_FIXTURES / "raw" / "post_call_transcription.json", "elevenlabs"))
    )
    ...
    assert not any(
        isinstance(e, StageObserved) and e.stage in {Stage.STT, Stage.LLM, Stage.TTS}
        for e in events
    )
...
def test_cartesia_turn_intervals_and_unplaced_ttfbs() -> None:
    ...
    assert any(
        e.stage is Stage.STT and e.metric is Metric.TTFB and e.value_ms == 90 for e in stages
    )
    assert any(
        e.stage is Stage.TTS and e.metric is Metric.TTFB and e.value_ms == 110 for e in stages
    )
