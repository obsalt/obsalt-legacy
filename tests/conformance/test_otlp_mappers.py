...
def test_pipecat_fixture_emits_grounding_and_intervals() -> None:
    ...
    assert any(isinstance(event, GroundingObserved) and "refund" in event.content.lower() for event in events)
    assert any(isinstance(event, GroundingObserved) and event.kind.value == "tool_result" for event in events)
    intervals = [
        event
        for event in events
...
def test_openinference_mapper_claims_and_decodes() -> None:
    ...
    assert any(
        isinstance(event, StageObserved) and event.placement is MeasurementPlacement.INTERVAL for event in events
    )
