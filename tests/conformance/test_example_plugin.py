...
class TestExampleAuth(AuthenticationConformanceTests):
    ...
    connection = ConnectionConfig(
        ...
        connection_id="c1",
        ingest_key_hash="x",
        secrets={"hmac_secret": "example-secret"},
    )
    valid_raw = (FIXTURES / "raw" / "call_ended.json").read_bytes()
...
def test_example_decode_keeps_stt_unplaced_and_populates_grounding() -> None:
    ...
    assert any(
        isinstance(e, GroundingObserved) and e.kind is GroundingKind.SYSTEM_PROMPT for e in events
    )
    assert any(
        isinstance(e, GroundingObserved) and e.kind is GroundingKind.USER_TEXT for e in events
    )
...
def test_example_signed_webhook_accepts_and_dedupes() -> None:
    ...
    headers = RawHeaders.from_mapping(
        {"x-obsalt-example-signature": sig, "content-type": "application/json"}
    )
...
def test_example_missing_secret_fail_closed() -> None:
    ...
    raw = b'{"call_id":"x"}'
    cfg = ConnectionConfig(
        org_id="acme", provider="example", connection_id="c1", ingest_key_hash="x", secrets={}
    )
    result = plugin.authenticate(
        raw, RawHeaders.from_mapping({"x-obsalt-example-signature": "ab"}).as_list(), cfg
    )
...
def test_example_rejects_synchronous_callbacks() -> None:
    plugin = ExamplePlugin()
    assert (
        plugin.classify(b'{"type":"assistant-request","call_id":"x"}')
        is ObservationalEventKind.REJECTED_SYNCHRONOUS
    )
