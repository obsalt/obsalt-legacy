...
def _signed_raw() -> tuple[bytes, dict[str, str]]:
    ...
    return raw, {"x-obsalt-example-signature": hmac_hex("s", raw), "content-type": "application/json"}
...
def test_crash_before_object_put_retries_cleanly() -> None:
    ...
    resolver = MemoryResolver()
    resolver.add(
        ConnectionConfig(org_id="acme", provider="example", connection_id="c", ingest_key_hash="", secrets={"hmac_secret": "s"}),
        "ik",
    )
...
def test_crash_after_put_resumes_incomplete_acceptance() -> None:
    ...
    assert result.created is True
    assert object_key_for("acme", "example", result.envelope.delivery_key, result.envelope.content_sha256) in objects.blobs  # type: ignore[union-attr]
...
def test_delete_by_call_cannot_be_replayed() -> None:
    ...
    call_id = items[0]["id"]
    source_call_id = client.get(f"/v1/calls/{call_id}", headers={"X-API-Key": "k"}).json()["source_call_id"]
    deleted = client.post(
...
def test_missing_hmac_fails_closed() -> None:
    ...
    resolver = MemoryResolver()
    resolver.add(
        ConnectionConfig(org_id="acme", provider="example", connection_id="c", ingest_key_hash="", secrets={}),
        "ik",
    )
    raw, headers = _signed_raw()
    result = receive_webhook(
