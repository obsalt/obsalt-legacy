...
def test_unplaced_stage_does_not_draw_waterfall() -> None:
    ...
    assert call.timeline_fidelity.value == "turn_level" or call.timeline_fidelity.value in {"call_level", "turn_level"}
...
def test_promote_cas_and_retain_prior_revision() -> None:
    ...
    sink.write(first)
    assert promote(pointers, first, expected=None, fact_frontier=frozenset({e1[0].fact_id or ""})).promoted
    e2 = [
        stamp_event(
    ...
    sink.write(second)
    assert promote(pointers, second, expected=first.revision, fact_frontier=frozenset({e2[0].fact_id or ""})).promoted
