...
def test_sdk_snapshot_roundtrip() -> None:
    recorder = CallRecorder(org_id="org", provider=Provider.OPENAI_REALTIME, call_id="sdk-1", agent_id="concierge")
    with recorder.turn(Speaker.USER, "book Friday") as turn:
        turn.stt_ms = 120
    with recorder.tool("create_booking", {"night": "Friday"}) as tool:
        tool.set_result({"confirmation": "HTL-1"})
    with recorder.turn(Speaker.AGENT, "Booked HTL-1") as turn:
        turn.llm_ms = 300
