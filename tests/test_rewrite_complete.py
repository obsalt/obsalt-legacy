"""Remaining rewrite-plan contracts: authz, worker isolation, range tombstones, health."""
...
EXAMPLE_RAW = (
    ROOT / "packages/obsalt-example/src/obsalt_example/fixtures/raw/call_ended.json"
)
...
def test_analyst_can_analyze_admin_cannot_rotate() -> None:
    ...
    owner = client.post("/v1/keys/rotate", headers={"X-API-Key": "dev-key"}, json={"overlap_seconds": 30})
...
def test_dead_air_and_truncated_llm_flags() -> None:
    ...
    revision = CallRevision(
        ...
        source_call_id="s",
        turns=[
            Turn(index=0, speaker=Speaker.USER, text="hello", started_at=t0, ended_at=t0 + timedelta(seconds=1)),
            Turn(
                index=1,
    ...
    kinds = {
        item["kind"]
        for item in next(row for row in analyze_tier1(length) if row.execution.analyzer_id == "flags").payload["flags"]
    }
    assert "truncated_llm" in kinds
