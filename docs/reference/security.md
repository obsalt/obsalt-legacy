...
signatures. Core retains raw multi-valued headers, rejects duplicates
for headers the plugin declares singleton, and passes the rest to
...
Deletion is verified across every managed store: Postgres control rows
and search documents delete synchronously; ClickHouse rows are masked
promptly, then physically removed; object storage purges current and
noncurrent versions; queues, DLQ references, rollups, caches, Parquet
manifests, and backup expiry are included. External copies cannot be
...
Outbound webhooks: HTTPS-only, Standard Webhooks signatures,
PII-minimal payloads, org-scoped outbox, no second logical event on
finalize retry.
...
Inbox / outbox age, orphan-blob count, decode failure rate by plugin,
DLQ depth, late-root frequency, active-revision promotion failure,
unmapped provider-code rate, unmapped-attribute rate, per-org queue
pressure, deletion backlog, tier-2 spend burn-down.
